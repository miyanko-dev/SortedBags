...
-- Bank base container (-1) plus the six bank bag slots (5..10). Only sorted while the bank window is open — moves are server-rejected otherwise.
local BANK_CONTAINERS = {BANK_CONTAINER or -1}
for i = 1, (NUM_BANKBAGSLOTS or 6) do
    tinsert(BANK_CONTAINERS, (NUM_BAG_SLOTS or 4) + i)
end

-- Bag category flags. A bag can carry any combination. Bit values are persisted in SortedBagsDB.bagFlags — don't change them. 0x080 is reserved (was MOUNT before mounts were merged into MISC) so future flags don't collide with legacy saves. Slot priority within a bag comes from FLAG_DEFAULT_ORDER below.
local FLAG = {
    HEARTHSTONE = 0x001,
    GEAR        = 0x002,
    CONSUMABLE  = 0x004,
...
-- Item → category flag. Categories map to Blizzard's Enum.ItemClass (literal IDs because Enum may not be populated at addon-load time in 1.15.x): 0 Consumable, 2 Weapon, 4 Armor, 5 Reagent, 6 Projectile, 7 Tradegoods, 9 Recipe, 12 Questitem, 13 Key, 15 Miscellaneous. Container/Projectile/Key/Miscellaneous all share the MISC bucket; Classic Era 1.15.x can't reliably distinguish mount items from other Misc items via subClassID. Quality 0 wins first so a gray sword lands in Junk, not Gear — same precedence Retail uses.
local function itemFilterFlag(itemID, classID, quality)
    ...
    -- Recipes live with Trade Goods so a profession bag tagged Trade Goods picks them up alongside the cloth / herbs / ore.
...
-- Specialty-bag item lists (ported from shirsig's SortBags v1.3.9). Quivers only accept arrows, soul bags only accept shards, etc. Hardware constraints from the bag itself — independent of user flags.
...
-- Bag itemID → specialty index, so specialty detection is one inventory lookup instead of comparing the bag name against every container's GetItemInfo (which silently fails on item-cache misses).
local SPECIALTY_BY_BAG_ITEM = {}
for idx, specialty in ipairs(SPECIALTY) do
    for _, itemID in ipairs(specialty.containers) do
...
-- Combat abort. Two paths can stop a running sort: (a) the PLAYER_REGEN_DISABLED event handler — normal path, fires between frames before the next coroutine resume. (b) the InCombatLockdown check inside sortPass — covers the race where the coroutine notices combat mid-sweep before the event reaches us, so we don't wait a full sweep / yield to stop. Both call showCombatAbortNotice, which is idempotent per sort so the raid-warning fires exactly once. Bags are left in their partially- sorted state on purpose — no rollback. Any in-flight cursor item is dropped so the player can fight unobstructed.
...
-- Walking away from the bank mid-sort: every further bank move is server-rejected, so stop immediately instead of letting the stall guard spin for five seconds.
...
local function isSafeNow()
    -- Merchant / mailbox / bank are safe: sorting only ever calls PickupContainerItem, and selling / attaching / depositing happens exclusively through UseContainerItem, which we never call.
...
local function itemAt(container, slot)
    ...
    if not itemName then
        -- GetItemInfo cache miss (common just after login / zoning in Classic Era). Return the link as an opaque key so the planner pins the slot in place rather than mistaking it for empty and shuffling another item on top.
        itemStacks[link] = 1
        itemFilterFlags[link] = 0
        return link, true
    end

    -- Item link is the identity. Different enchant / random suffix means a different link, which the server already treats as unstackable.
    local key = link
    ...
    -- classID comes before subClassID so weapons (classID 2) sort ahead of armor (classID 4) inside GEAR, and items inside MISC / JUNK group by their underlying class (containers, projectiles, keys, …).
    itemSortKeys[key] = {
    ...
    if itemID then
        for idx, specialty in ipairs(SPECIALTY) do
            if specialty.items[itemID] then
...
local function buildPlan()
    ...
    -- Container traversal order. rightToLeft now means "rightmost bag first" (backpack, bag1, ...); off means "leftmost bag first" (bag4, bag3, ..., backpack). Slots inside each bag are always visited top-left → bottom-right so items end up in slot order.
    ...
    for _, container in ipairs(containerOrder) do
        ...
        for slot = 1, n do
            ...
            if key then
                local slotInfo = C_Container.GetContainerItemInfo(container, slot)
                local locked = uncached or (slotInfo and slotInfo.isLocked)
                entry.item  = key
                entry.count = slotInfo and slotInfo.stackCount or 1
                if locked then
                    -- Pin locked / uncached items in place so the planner sorts around them. Classic Era can leave items locked after a failed in-combat equip or a dismissed BoE bind popup; the lock is server-side and only clears on relog. The previous build aborted the entire plan on any locked slot and retried forever, which made the sort button look dead until relog.
    ...
    sort(items, function(a, b) return lessThan(itemSortKeys[a], itemSortKeys[b]) end)
    ...
    -- Phase 2: per-bag fill. Walk each tagged bag's flags in the canonical FLAG_DEFAULT_ORDER, which puts Hearthstone first and Misc/Mounts second — so those items always claim a bag's earliest slots whenever the bag carries those tags. Slots inside the bag fill top-left → bottom-right because they were inserted into the model in slot 1..N order.
    for _, container in ipairs(containerOrder) do
        local bagFlags = ns.GetBagFlags(container)
        if bagFlags ~= 0 then
...
-- Move primitive. Cursor is left clean after every call. The coroutine yields once per sortPass sweep, not per move, so this runs many times back-to-back inside one frame.
local function move(src, dst)
    -- One move per slot per sweep. Without this, two moves in the same frame can target the same slot before the server's lock state has propagated, silently dropping the second move and desyncing our model.
    ...
    -- Empty-but-locked destination: GetContainerItemInfo returns nil for empty slots, so the isLocked check above can't see a lock that's still pending from a move that just emptied it. The client rejects the drop and leaves the source item on the cursor — put it back and report failure instead of recording a swap that never happened. (Occupied slots are covered by the dstInfo.isLocked check.)
...
local function sortPass()
    while true do
        -- Combat check once per sweep: a sweep runs synchronously, so InCombatLockdown can't flip mid-iteration. The notice is idempotent so the event handler won't double-alert.
        ...
        -- Index candidate sources by item once per sweep instead of rescanning the whole model for every destination. Slots mutated mid-sweep are always in touchedSlots, so stale entries are rejected at use time — same outcome as a fresh scan.
        local sourcesByItem = {}
        for _, src in ipairs(model) do
            ...
            if dst.targetItem and (dst.item ~= dst.targetItem or (dst.count or 0) < dst.targetCount) then
                ...
                if candidates and not touchedSlots[slotKey(dst.container, dst.position)] then
                    ...
                    for _, src in ipairs(candidates) do
                        ...
                        if srcOK then
                            -- Count distance first (splits/merges stay minimal for stackables), then prefer a mutual swap (src wants exactly what dst holds): one move settles two slots. For gear every distance is 0, so the swap preference decides there and cuts the lock round trips on gear-heavy inventories.
...
-- Public entry + engine
function ns.Sort()
    -- Treat a suspended process whose engine isn't actually running as stale and fall through to a fresh sort. This guards against any code path that nils the engine without nilling process — without it, a stuck "in progress" state would silently swallow every click until /reload.
    ...
    -- Bags and bank sort as separate groups so items never cross the bag/bank boundary — sorting must not deposit or withdraw.
...
-- After a batch of moves, nothing can progress until the server acks a lock release, so resuming every frame just burns CPU on no-op sweeps. ITEM_UNLOCKED / BAG_UPDATE_DELAYED mark exactly those acks; the 0.25s fallback rescues the sort if an expected event never arrives.
engine:RegisterEvent("ITEM_UNLOCKED")
engine:RegisterEvent("BAG_UPDATE_DELAYED")
-- Bank base slots (container -1) signal through PLAYERBANKSLOTS_CHANGED rather than BAG_UPDATE_DELAYED.
...
engine:SetScript("OnUpdate", function(self, elapsed)
    ...
    -- A healthy sort moves something every lock round trip. Five seconds with zero moves means the plan can't be reached (mid-sort user interference, permanently locked slot); abort with a message instead of sweeping forever — the next click replans from live bag state.
