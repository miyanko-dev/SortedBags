...
-- Flag labels. Hearthstone, Trade Goods, and Junk pull Blizzard's localized strings; the rest are literal English because the game's singular class names read awkwardly as bag categories.
...
-- Portrait dimming: full grey when ignored, subtle desaturate when the bag has any category flag, otherwise normal.
...
-- MenuUtil.CreateContextMenu is the same menu system and skin as the unit-portrait right-click menu (MenuStyle2, common-dropdown-c-bg): it opens at the cursor, keeps itself on screen, closes on ESC, click-out, or when the owning portrait hides, and checkbox rows refresh in place so the menu stays open while toggling. Vanilla's variant shows a yellow checkmark when an option is on and nothing when it's off (see Blizzard_Menu/Vanilla/MenuVariants.lua) -- the native on/off cue known from the tracking and unit menus.
local function showBagMenu(anchor, frame)
    ...
    MenuUtil.CreateContextMenu(anchor, function(_, root)
        ...
        -- Global options live on the backpack menu only -- they apply to everything, so duplicating them on every bag would be noise.
        if bag == 0 then
            root:CreateDivider()
...
-- Portrait click handler. Backpack: left-click sorts, right-click menu. Other bags: right-click menu only.
-- We bind directly to Blizzard's $parentPortraitButton instead of layering our own overlay on top: an overlay child with mouse enabled took OnEnter (tooltips worked) but, for reasons that aren't worth chasing, didn't deliver LeftButton OnClick reliably here, while RightButton did. PortraitButton is a regular Button with no OnClick of its own in the 1.15.x XML, so SetScript("OnClick") is a safe addition.
-- ContainerFrame1..13 are also a pool in Classic Era 1.15.x: GetOpenFrame() hands the first hidden frame to whichever bag opens next, so the same ContainerFrameN can host the backpack one show and bag #2 the next. That means we can't bake "is this the backpack?" into a closure at attach time — re-read frame:GetID() on every click / hover.
local function attachPortraitClick(frame)
    ...
    -- Dim overlay for hover/press feedback on the backpack only. SetVertexColor on the native portrait texture is silently no-op'd in Classic Era 1.15 (the C_Container.SetBagPortraitTexture pipeline ignores subsequent Lua color changes), so paint a dim layer on top. Masked to the same circular alpha Blizzard uses so the dim matches the portrait socket.
    ...
    -- OnMouseDown / OnMouseUp give a stronger "pressed" dim distinct from hover. OnClick fires on release, so this feedback runs in parallel with sort/menu.
